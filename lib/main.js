...
function check_director() {
    ...
    execute("search/person", { "query": director_name_input })
        ...
        movie_set.descriptions.sort((lhs, rhs) => lhs.name.localeCompare(rhs.name));
        let table_html = "<tr><th></th>" + movie_set.descriptions.map((description) => "<th>" + description.name + "</th>").join("") + "</tr>";
